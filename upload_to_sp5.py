...
try:
    ...
    # URL for deleting the existing file
    delete_url = f"{sharepoint_url}/{site_url}/_api/web/GetFileByServerRelativeUrl('{folder_path}/{remote_file_name}')"
    delete_headers = {'Accept': 'application/json;odata=verbose', 'X-RequestDigest': form_digest_value, 'IF-MATCH': '*', 'X-HTTP-Method': 'DELETE'}

    # Attempt to delete the file
    delete_response = session.post(delete_url, headers=delete_headers, verify=False)
